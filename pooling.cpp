...
float input_data[2][3][5] = {
    {
        {1.0, 2.0, 4.0, 5.0, -2.3},
        {12.0, -82.0, 54.0, 15.0, -2.3},
        {102.0, -272.0, 114.0, -14.3, -2.3},
    },
    {
        {1.0, 2.0, 4.0, 5.0, -2.3},
        {12.0, -82.0, 54.0, 15.0, -2.3},
        {102.0, -172.0, 114.0, -15.0, -2.3},
    }
};
...
public:
    void run() {
        bufferSize = sizeof(float) * CHANNELS * HEIGHT * WIDTH;
    ...
    void saveRenderedImage() {
        ...
        vkMapMemory(device, bufferMemory2, 0, bufferSize, 0, &mappedMemory);
        float* output = (float*)mappedMemory;

        for(int c = 0; c < CHANNELS; c++){
            for(int i = 0; i < HEIGHT; i++){
                for(int j = 0; j < WIDTH; j++){
                    std::cout << output[j + (i * WIDTH) + (c * WIDTH * HEIGHT)] << " ";
                }
                std::cout << std::endl;
            }
            std::cout << std::endl;
    ...
    void createBuffer() {
        ...
        bufferCreateInfo2.size = bufferSize; // buffer size in bytes. 
        bufferCreateInfo2.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT; // buffer is used as a storage buffer.
        bufferCreateInfo2.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // buffer is exclusive to a single queue family at a time. 

        VK_CHECK_RESULT(vkCreateBuffer(device, &bufferCreateInfo2, NULL, &buffer2)); // create buffer.

        VkMemoryRequirements memoryRequirements2;
        vkGetBufferMemoryRequirements(device, buffer2, &memoryRequirements2);
        
        VkMemoryAllocateInfo allocateInfo2 = {};
        allocateInfo2.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocateInfo2.allocationSize = memoryRequirements2.size; // specify required memory.
        allocateInfo2.memoryTypeIndex = findMemoryType(
            memoryRequirements2.memoryTypeBits, 
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);

        VK_CHECK_RESULT(vkAllocateMemory(device, &allocateInfo2, NULL, &bufferMemory2)); // allocate memory on device.
    ...
    void createDescriptorSetLayout() {
        ...
        std::vector<VkDescriptorSetLayoutBinding> bindings = { descriptorSetLayoutBinding, descriptorSetLayoutBinding2 };

        VkDescriptorSetLayoutCreateInfo descriptorSetLayoutCreateInfo = {};
        descriptorSetLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
        descriptorSetLayoutCreateInfo.bindingCount = (uint32_t) bindings.size(); // only a single binding in this descriptor set layout. 
    ...
    void createDescriptorSet() {
        ...
        descriptorBufferInfo2.buffer = buffer2;
        descriptorBufferInfo2.offset = 0;
        descriptorBufferInfo2.range = bufferSize;

        VkWriteDescriptorSet writeDescriptorSet2 = {};
        writeDescriptorSet2.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        writeDescriptorSet2.dstSet = descriptorSet; // write to this descriptor set.
        writeDescriptorSet2.dstBinding = 1; // write to the first, and only binding.
        writeDescriptorSet2.descriptorCount = 1; // update a single descriptor.
        writeDescriptorSet2.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER; // storage buffer.
    ...
    void createComputePipeline() {
        uint32_t filelength;
        uint32_t* code = readFile(filelength, "../shaders/pooling.comp.spv");
        ...
        pushConstant.size = sizeof(Variables);
    ...
    void createCommandBuffer() {
        ...
        void* data_pointer;
        vkMapMemory(device, bufferMemory, 0, bufferSize, 0, &data_pointer);
        memcpy(data_pointer, &input_data[0][0][0], bufferSize);
        vkUnmapMemory(device, bufferMemory);

        Variables variables;
        variables.filter_x_size = 3;
        variables.filter_y_size = 3;
        variables.input_channels = 2;
        variables.input_height = 3;
        variables.input_width = 5;
        variables.maxPool = 1;
        variables.padding = 1;

        vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(Variables), &variables);
        ...
        vkCmdDispatch(
            ...
            (uint32_t)ceil(CHANNELS / float(WORKGROUP_SIZE))
            // (uint32_t)ceil(CHANNELS / float(WORKGROUP_SIZE))
        );
    ...
    void cleanup() {
        ...
        vkFreeMemory(device, bufferMemory2, NULL);

        vkDestroyBuffer(device, buffer, NULL);	
        vkDestroyBuffer(device, buffer2, NULL);	
