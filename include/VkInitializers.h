...
namespace vkInit {
    ...
    VkShaderModuleCreateInfo createShaderModuleCreateInfo(size_t size, const uint32_t *data);

    VkPipelineShaderStageCreateInfo createPipelineShaderStageCreateInfo(VkShaderStageFlagBits bits, VkShaderModule &shaderModule);
    ...
    VkPipelineRasterizationStateCreateInfo createPipelineRasterizationStateCreateInfo(VkPolygonMode polygonMode);
    ...
    VkBufferCreateInfo createBufferCreateInfo(VkDeviceSize size, VkBufferUsageFlags usageFlags);

    VkPushConstantRange createPushConstantRange(uint32_t size, VkShaderStageFlags stageFlags);
}